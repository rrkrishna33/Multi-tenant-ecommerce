/**
 * The phone and WhatsApp marks used beside the shop's numbers in the header.
 *
 ...
 * its own phone number. They inherit `currentColor`, so they sit correctly on
 * the header's brand background whatever colour a shop picks.
 *
 * `aria-hidden` on both -- the number beside them is the label, and a screen
 * reader announcing "image, phone, 98420 12345" reads worse than the number
 * alone.
 */
...
export function whatsappHref(number: string): string {
  return `https://wa.me/91${number.replace(/\D/g, "").slice(-10)}`;
}
